...
router.get("/stats/summary", verifyToken, (req, res) => {
    const queries = {
        ...
        monthly6:        `SELECT DATE_FORMAT(payment_date,'%b %Y') AS label, MONTH(payment_date) AS mo, YEAR(payment_date) AS yr, SUM(amount) AS total
...
router.post("/", verifyToken, (req, res) => {
    const {
        ...
        payment_for, status, months_covered, notes
    } = req.body;

    if (!member_id || !amount || !payment_date)
        return res.status(400).json({ success: false, message: "member_id, amount and payment_date required" });

    const sql = `INSERT INTO payments
        (member_id, amount, payment_date, payment_method, payment_for, status, months_covered, notes, received_by)
        VALUES (?,?,?,?,?,?,?,?,?)`;
...
router.put("/:id", verifyToken, (req, res) => {
    ...
    db.query(sql, [
        member_id, amount, payment_date, payment_method,
        payment_for, status, months_covered, notes, req.params.id
    ], (err, result) => {
        if (err) return res.status(500).json({ success: false, message: "DB Error" });
