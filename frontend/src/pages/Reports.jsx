...
function StatCard({ icon:Icon, label, value, color="#60a5fa", sub, onClick, clickable }) {
  return (
    <div
      onClick={onClick}
      style={{ background:"var(--bg-surface)", border:`1px solid ${clickable?"rgba(96,165,250,0.3)":"var(--border-subtle)"}`, borderRadius:"var(--radius-lg)", padding:"20px 24px", display:"flex", alignItems:"center", gap:"16px", cursor:clickable?"pointer":"default", transition:"all 0.15s" }}
    ...
    >
      ...
      <div>
        ...
        {clickable && <div style={{ fontSize:"10px", color:"#60a5fa", marginTop:"3px" }}>Click to drill down →</div>}
      </div>
    </div>
  );
}
...
export default function Reports({ onLogout }) {
  ...
  // drill: { mode, month, year }
  // mode: "month" | "this-month" | "last-year" | "all-time" | "method"
  // month used for month number OR method name
  ...
  const exportPDF = () => {
    ...
    if (activeTab==="revenue" && data?.monthly) {
      autoTable(doc, { startY:38, head:[["Month","Revenue (₹)","Transactions"]],
      ...
      <>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:"20px", marginBottom:"20px" }}>
          <ChartCard title={`New Members Per Month — ${year}`}>
            <Line data={{ labels:MONTHS, datasets:[{ label:"New Members", data:monthlyNew, borderColor:chartColors.green, backgroundColor:chartColors.greenBg, borderWidth:2, fill:true, tension:0.4, pointBackgroundColor:chartColors.green }] }} options={chartOpts()} />
          </ChartCard>
          <ChartCard title="Membership Type">
            <Doughnut data={{ labels:byType?.map(t=>t.membership_type)||[], datasets:[{ data:byType?.map(t=>t.count)||[], backgroundColor:[chartColors.blue,chartColors.green,chartColors.purple,chartColors.yellow,chartColors.red], borderWidth:0 }] }}
              options={{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{ position:"bottom", labels:{ color:"#9ca3af", padding:16, font:{ size:11 } } }, tooltip:{ ...chartOpts().plugins.tooltip } } }} />
          </ChartCard>
        </div>
        <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border-subtle)", borderRadius:"var(--radius-lg)", overflow:"hidden" }}>
          <div style={{ padding:"16px 20px", borderBottom:"1px solid var(--border-subtle)" }}>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead><tr style={{ borderBottom:"1px solid var(--border-subtle)" }}>{["Month","New Members"].map(h => <th key={h} style={{ padding:"12px 20px", textAlign:"left", fontSize:"11px", fontWeight:600, color:"var(--text-muted)", textTransform:"uppercase", letterSpacing:"0.08em" }}>{h}</th>)}</tr></thead>
            <tbody>
              {MONTHS.map((m,i) => { const r=monthly?.find(x=>x.month===i+1); const c=r?.new_members||0; return (
                <tr key={m} style={{ borderBottom:"1px solid var(--border-subtle)" }} onMouseEnter={e=>e.currentTarget.style.background="var(--bg-elevated)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                  <td style={{ padding:"12px 20px", color:"var(--text-primary)", fontSize:"13px" }}>{m}</td>
      ...
      <>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"20px", marginBottom:"20px" }}>
          <ChartCard title={`Monthly Attendance — ${year}`}>
            <Bar data={{ labels:MONTHS, datasets:[{ label:"Present", data:monthlyPresent, backgroundColor:chartColors.purpleBg, borderColor:chartColors.purple, borderWidth:2, borderRadius:6 }] }} options={chartOpts()} />
          </ChartCard>
          <ChartCard title="Last 30 Days Daily Trend">
            <Line data={{ labels:weekly?.map(w=>w.day)||[], datasets:[{ label:"Check-ins", data:weekly?.map(w=>w.count)||[], borderColor:chartColors.yellow, backgroundColor:"rgba(251,191,36,0.1)", borderWidth:2, fill:true, tension:0.4, pointBackgroundColor:chartColors.yellow }] }} options={chartOpts()} />
          </ChartCard>
        </div>
        {topMembers?.length>0 && (
          <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border-subtle)", borderRadius:"var(--radius-lg)", overflow:"hidden" }}>
            <div style={{ padding:"16px 20px", borderBottom:"1px solid var(--border-subtle)" }}>
              <h3 style={{ fontFamily:"var(--font-display)", fontSize:"14px", fontWeight:700, color:"var(--text-primary)", margin:0 }}>Top 5 Most Active Members</h3>
            </div>
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              <thead><tr style={{ borderBottom:"1px solid var(--border-subtle)" }}>{["Rank","Member","Membership","Visits"].map(h=><th key={h} style={{ padding:"12px 20px", textAlign:"left", fontSize:"11px", fontWeight:600, color:"var(--text-muted)", textTransform:"uppercase", letterSpacing:"0.08em" }}>{h}</th>)}</tr></thead>
              <tbody>
                {topMembers.map((m,i) => (
                  <tr key={i} style={{ borderBottom:"1px solid var(--border-subtle)" }} onMouseEnter={e=>e.currentTarget.style.background="var(--bg-elevated)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                    ...
                    <td style={{ padding:"12px 20px" }}><span style={{ padding:"3px 10px", borderRadius:"20px", fontSize:"11px", fontWeight:600, background:"rgba(96,165,250,0.1)", color:"#60a5fa" }}>{m.membership_type}</span></td>
    ...
    <>
      <div style={{ display:"flex", minHeight:"100vh", background:"var(--bg-base)", fontFamily:"var(--font-body)" }}>
        ...
        <main style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
          ...
          {/* Header */}
          <div style={{ padding:"24px 32px", borderBottom:"1px solid var(--border-subtle)", background:"var(--bg-surface)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div>
              <h1 style={{ fontFamily:"var(--font-display)", fontSize:"22px", fontWeight:800, color:"var(--text-primary)", margin:0 }}>Reports & Analytics</h1>
              <p style={{ color:"var(--text-muted)", fontSize:"13px", margin:"4px 0 0" }}>Visual insights from your gym data</p>
            </div>
            <div style={{ display:"flex", gap:"10px", alignItems:"center" }}>
              <select value={year} onChange={e=>setYear(Number(e.target.value))} style={{ padding:"9px 14px", background:"var(--bg-elevated)", border:"1px solid var(--border-default)", borderRadius:"var(--radius-md)", color:"var(--text-primary)", fontSize:"13px", cursor:"pointer", outline:"none" }}>
                {years.map(y=><option key={y} value={y}>{y}</option>)}
              </select>
              <button onClick={exportPDF} style={{ display:"flex", alignItems:"center", gap:"8px", padding:"9px 16px", background:"rgba(248,113,113,0.1)", border:"1px solid rgba(248,113,113,0.2)", borderRadius:"var(--radius-md)", color:"#f87171", cursor:"pointer", fontSize:"13px", fontWeight:600 }}>
                <FaFilePdf/> PDF
              </button>
              <button onClick={exportExcel} style={{ display:"flex", alignItems:"center", gap:"8px", padding:"9px 16px", background:"rgba(52,211,153,0.1)", border:"1px solid rgba(52,211,153,0.2)", borderRadius:"var(--radius-md)", color:"#34d399", cursor:"pointer", fontSize:"13px", fontWeight:600 }}>
                <FaFileExcel/> Excel
              </button>
            </div>
          </div>
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"24px 32px" }}>
            {/* Tabs */}
            <div style={{ display:"flex", gap:"4px", marginBottom:"24px", background:"var(--bg-surface)", border:"1px solid var(--border-subtle)", borderRadius:"var(--radius-lg)", padding:"6px", width:"fit-content" }}>
              {TABS.map(({ key, label, icon:Icon }) => (
                <button key={key} onClick={()=>setActiveTab(key)} style={{ display:"flex", alignItems:"center", gap:"8px", padding:"9px 20px", borderRadius:"var(--radius-md)", border:"none", cursor:"pointer", fontSize:"13px", fontWeight:600, background:activeTab===key?"var(--bg-active)":"transparent", color:activeTab===key?"var(--text-primary)":"var(--text-muted)", transition:"all 0.15s" }}>
